/******************************************************************
 3. LQ DATA
******************************************************************/
// main file edits
tempfile lq_main_file
use  "${LQ_main_form}", clear
keep if ind_id !=  "##N/A##" & !missing(status)

replace ind_id = trim(ind_id)

// correct wrong IDs
replace ind_id = "1" if interview__key == "22-46-38-80" & ind_id == "0442_4022_1_0031_0001"
replace ind_id = "4" if interview__key == "01-97-47-26" & ind_id == "223"
replace ind_id = "1" if interview__key == "60-96-80-95" & ind_id == "248"
replace ind_id = "2" if interview__key == "11-72-99-70" & ind_id == "43"
replace ind_id = "3" if interview__key == "92-52-12-18" & ind_id == "46"
replace ind_id = "5" if interview__key == "67-47-38-40" & ind_id == "05"
replace ind_id = "6" if interview__key == "13-89-98-16" & ind_id == "06"
replace ind_id = "8" if interview__key == "28-07-57-69" & ind_id == "3"
replace ind_id = "9" if interview__key == "32-52-84-36" & ind_id == "4"
replace ind_id = "5" if interview__key == "18-37-39-93" & ind_id == "10"

drop if inlist(interview__key, "01-56-70-38", "14-88-27-82", "32-15-18-59", "30-08-15-07", "01-56-70-38")
duplicates list LQ_ID ind_id

save `lq_main_file'

// load person listing file
use "${LQ_person_listing}" , clear

// drop duplicates
drop if inlist(interview__key, "98-24-79-01", "99-38-64-97", "81-28-77-31")
isid LQ_ID

keep interview__key interview__id GHI_ISLAND_CODE PSU GHI_BLOCK_CODE GHI_STRUCTURE DWELLING_ID SELECTION LQ_ID nbslct

// merge with selection
merge 1:m interview__key using "${LQ_person_listing_selection}", keepusing(ind__id indsta ind_nme)
// assert _merge != 2
keep if inlist(_merge, 1, 3)
drop _merge

rename ind_nme name
rename ind__id ind_id
tostring ind_id, replace

// use  "${LQ_main_form}", clear 
replace ind_id = trim(ind_id)
drop interview__key interview__id

merge 1:1 LQ_ID ind_id using `lq_main_file', keepusing(status interview__key ind_nme)

keep if inlist(_merge,1,3)
drop _merge

replace indsta=1 if indsta==1

gen finished = indsta==1
// keep if indsta == 1

bysort LQ_ID: egen total_finished = total(finished)
keep GHI_STRUCTURE DWELLING_ID LQ_ID SELECTION PSU GHI_ISLAND_CODE GHI_BLOCK_CODE nbslct total_finished

bysort LQ_ID : keep if _n==1

gen completed = nbslct == total_finished

save "lq_file_cleaned.dta", replace